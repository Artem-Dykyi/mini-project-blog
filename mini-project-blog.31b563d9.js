...
document.addEventListener("submit", async (event)=>{
    if (event.target.classList.contains("createCommentForm")) {
        ...
        if (!text) return;
        (0, _createComment.createComment)(postId, {
            text
        });
        input.value = "";
        // Повторно рендеримо пости з оновленими коментарями
        const updatedPosts = (0, _getPosts.getPosts)();
        document.querySelector(".post-list").innerHTML = (0, _renderPosts.renderPosts)(updatedPosts);
    }
});

},{"./createComment":"bvT97","./createPost":"5kP8d","./deletePost":"c5e6f","./getPosts":"i41ol","./updatePost":"1p1uB","./renderPosts":"bD6By"}],"bvT97":[function(require,module,exports,__globalThis) {
...
const getPosts = async ()=>{
    try {
        return await fetch("http://localhost:3000/peopleBlog").then((response)=>response.json());
    } catch (error) {
        console.error(error);
    }
};
...
const renderPosts = (posts)=>posts.map((post)=>`<li class="post-item">
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["5j6Kf","a0t4e"], "a0t4e", "parcelRequire9797", {})
